  ...
  if (Notification?.permission === "granted") {
    ...
    Notification.requestPermission().then((status) => {
      if (status === "granted") {
        ...
        alert(
          "You have to give notification permission to get the check out notification",
        );
      }
    });
  } else {
...
dc.addEventListener("click", () => {
  if (
    window.confirm(
      `You are about to remove ${TCN} ${document.getElementById("product-name").innerHTML} from your cart`,
    )
  ) {
